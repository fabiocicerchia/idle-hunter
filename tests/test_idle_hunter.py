...
def test_lb_score_zero_when_targets_exist() -> None:
...
def test_lb_score_boosted_by_zero_traffic() -> None:
    lb = {"CreatedTime": days_ago(90), "Type": "application"}
...
def test_snapshot_and_ami_scores() -> None:
...
def test_iac_managed_tags_lower_the_score() -> None:
    ...
    assert "IaC-managed" in f.note


def test_price_falls_back_to_estimates_without_live_lookup() -> None:
...
def test_render_sorts_and_filters() -> None:
    fs = [
    ...
    assert "eip-1" in out
    assert "vol-1" not in out
    assert "1 finding(s)" in out


def test_eni_score_zero_when_a_service_owns_it() -> None:
...
def test_rds_score_treats_missing_metrics_as_unknown() -> None:
...
def test_rds_engine_name_maps_families_and_refuses_to_guess() -> None:
...
def test_rds_shape_carries_class_engine_deployment_and_licence() -> None:
    shape = rds_shape(
        {
